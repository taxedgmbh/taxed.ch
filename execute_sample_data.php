 ...
 * This script will run on your Hostinger server and insert sample data
 */
...
try {
    ...
    echo "<h1>🗄️ Taxed.ch Database - Sample Data Execution</h1>";
    echo "<p>✅ Database connection successful!</p>";
    echo "<p>📊 Database: {$db_config['dbname']}</p>";
    echo "<p>👤 User: {$db_config['username']}</p>";
...
// Function to show table count
function showTableCount($pdo, $table, $description) {
    try {
        $stmt = $pdo->query("SELECT COUNT(*) as count FROM $table");
        $result = $stmt->fetch();
        $count = $result['count'];
        echo "<p>📊 $description: $count records</p>";
        return $count;
    } catch (PDOException $e) {
        echo "<p>❌ $description: Error - " . $e->getMessage() . "</p>";
        return 0;
...
echo "<h2>🚀 Starting Sample Data Insertion...</h2>";

// Step 1: Insert additional admin users
$admin_sql = "INSERT INTO admin_users (username, email, password_hash, first_name, last_name, role) VALUES
...
executeSQL($pdo, $admin_sql, "Admin Users Insertion");

// Step 2: Insert sample clients
$clients_sql = "INSERT INTO clients (email, password_hash, first_name, last_name, company, phone, address, city, postal_code, country, tax_id, status) VALUES
...
executeSQL($pdo, $clients_sql, "Sample Clients Insertion");

// Step 3: Insert sample tax cases
$cases_sql = "INSERT INTO tax_cases (client_id, case_number, tax_year, case_type, status, priority, description, assigned_to, due_date, created_at) VALUES
...
executeSQL($pdo, $cases_sql, "Tax Cases Insertion");

// Step 4: Insert sample documents
$documents_sql = "INSERT INTO documents (client_id, case_id, filename, original_filename, file_path, file_size, mime_type, document_type, description, uploaded_by, created_at) VALUES
...
executeSQL($pdo, $documents_sql, "Documents Insertion");

// Step 5: Insert sample messages
$messages_sql = "INSERT INTO messages (client_id, case_id, sender_type, subject, message, is_read, created_at) VALUES
...
executeSQL($pdo, $messages_sql, "Messages Insertion");

// Step 6: Insert sample appointments
$appointments_sql = "INSERT INTO appointments (client_id, title, description, appointment_date, duration_minutes, status, meeting_type, meeting_link, notes, created_at) VALUES
...
executeSQL($pdo, $appointments_sql, "Appointments Insertion");

// Step 7: Insert sample audit log entries
$audit_sql = "INSERT INTO audit_log (user_id, user_type, action, table_name, record_id, old_values, new_values, ip_address, user_agent, created_at) VALUES
...
executeSQL($pdo, $audit_sql, "Audit Log Insertion");

echo "<h2>🔍 Verifying Data Insertion...</h2>";

// Show final counts
$total_records = 0;
$total_records += showTableCount($pdo, 'admin_users', 'Admin Users');
$total_records += showTableCount($pdo, 'clients', 'Clients');
$total_records += showTableCount($pdo, 'tax_cases', 'Tax Cases');
$total_records += showTableCount($pdo, 'documents', 'Documents');
$total_records += showTableCount($pdo, 'messages', 'Messages');
$total_records += showTableCount($pdo, 'appointments', 'Appointments');
$total_records += showTableCount($pdo, 'audit_log', 'Audit Log Entries');

echo "<h2>🎉 SAMPLE DATA INSERTION COMPLETE!</h2>";
echo "<p><strong>Total records in database: $total_records</strong></p>";
...
echo "<ul>";
echo "<li>john.doe@email.com (password: admin123)</li>";
echo "<li>jane.smith@email.com (password: admin123)</li>";
echo "<li>robert.johnson@email.com (password: admin123)</li>";
echo "<li>sarah.wilson@email.com (password: admin123)</li>";
echo "<li>michael.brown@email.com (password: admin123)</li>";
echo "<li>emma.davis@email.com (password: admin123)</li>";
echo "<li>david.miller@email.com (password: admin123)</li>";
echo "<li>lisa.garcia@email.com (password: admin123)</li>";
echo "</ul>";
echo "</ol>";
