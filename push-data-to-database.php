 ...
 * This script will automatically insert all sample data into the database
 */
...
try {
    ...
    echo "<h1>🗄️ Taxed.ch Database - Sample Data Insertion</h1>";
...
foreach ($sample_data as $table => $queries) {
    ...
    foreach ($queries as $query) {
        try {
            $pdo->exec($query);
            $success_count++;
            $results[] = "✅ $table: Data inserted successfully";
            echo "<p>✅ $table: Data inserted successfully</p>";
        } catch (PDOException $e) {
            $error_count++;
...
echo "<tr style='background-color: #f0f0f0;'><th>Table</th><th>Description</th><th>Record Count</th></tr>";
...
foreach ($table_counts as $table => $description) {
    try {
        ...
        $result = $stmt->fetch();
        $count = $result['count'];
        echo "<tr><td>$table</td><td>$description</td><td style='text-align: center; font-weight: bold;'>$count</td></tr>";
    } catch (PDOException $e) {
        echo "<tr><td>$table</td><td>$description</td><td style='color: red;'>❌ Error</td></tr>";
    }
}
echo "</table>";
...
echo "<ul>";
echo "<li>✅ Successful insertions: $success_count</li>";
echo "<li>❌ Errors encountered: $error_count</li>";
echo "<li>🗄️ Database: {$db_config['dbname']}</li>";
echo "<li>👤 Database User: {$db_config['username']}</li>";
echo "</ul>";

if ($error_count == 0) {
    echo "<h1>🎉 SAMPLE DATA INSERTION COMPLETE!</h1>";
    echo "<p><strong>All sample data has been successfully inserted into your database!</strong></p>";
    ...
    echo "<h1>⚠️ INSERTION COMPLETED WITH ERRORS</h1>";
    echo "<p>Please review the errors above. Some data may not have been inserted correctly.</p>";
}
